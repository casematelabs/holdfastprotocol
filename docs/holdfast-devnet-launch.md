...
Holdfast Protocol is our answer. Today, `@holdfastprotocol/sdk@0.1.1-devnet` is published and the programs are live on Solana devnet. Here is what it is, how it works, and how to start building with it.
...
The `devnet` dist-tag points to `0.1.1-devnet`. The `latest` tag is intentionally unset — this SDK is devnet-only until the external audit completes.
...
We built a working end-to-end demo that walks through the full flow in a single script:
...
`@holdfastprotocol/sdk@0.1.1-devnet` published to npm. Programs deployed on Solana devnet.

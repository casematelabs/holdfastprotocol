...
| Funded devnet keypair | See Step 3 setup below |
...
## Step 7 — Timed mode: run a keeper (required for auto-release)

If you created a pact with `releaseCondition: { kind: 'timed', ... }`, the SDK will **not** automatically call `auto_release` when the time lock expires. A timed pact in `Locked` status stays locked indefinitely until someone submits the `auto_release` instruction on-chain. Without a keeper, beneficiaries will not receive funds even after the expiry date passes.

**Recommended approach for devnet:** run the reference keeper script, which polls for your expired timed pacts and fires `auto_release` for each one.
...
The script polls the indexer every 5 minutes (configurable via `POLL_INTERVAL_SECS`). It only processes pacts where you are the beneficiary and `timeLockExpiresAt` has passed.

**Dry-run first to confirm candidates:**
...
> **Note:** The `client.escrow.autoRelease()` SDK method is on the roadmap. Once it ships, you can call it directly instead of running the keeper script. The keeper script is the reference implementation in the interim.

> **Task and milestone pacts do not need a keeper.** The `auto_release` instruction only works for timed pacts (`auto_release_on_expiry = true`). If you accidentally run the keeper against a task pact, the program will reject the instruction — no funds are at risk.
...
| Auto-release (timed mode) | Run the keeper script — see Step 7 |
| Open dispute | `client.escrow.openDispute(escrowPDA, reason?)` |

The **full runnable script** covering all these steps is at [`sdk/examples/quickstart.ts`](../sdk/examples/quickstart.ts). Run it with:
...
- [Integration Guide](./integration-guide.md) — PDA derivations, program addresses, SDK deep-dive
- [ElizaOS Integration Guide](./elizaos-integration-guide.md) — plug Holdfast into an ElizaOS agent
- [Reputation Composability Guide](./reputation-composability.md) — gate your protocol on Holdfast reputation (off-chain SDK and on-chain CPI)
- [Troubleshooting Reference](./troubleshooting.md) — error codes and recovery paths
- [Runnable quickstart script](../sdk/examples/quickstart.ts) — copy-paste to your terminal
